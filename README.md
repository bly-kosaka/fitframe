...
- アップロード → サイズ・形状・出力形式の設定 → 自動配置結果の確認・個別調整 → ダウンロード、という単一画面のウィザード形式です。
...
- デスクトップ・モバイル両対応（全画面レスポンシブ）。

## 機能

| 画面 | 内容 |
|---|---|
| アップロード | ドラッグ&ドロップ／ファイル選択／クリップボード貼り付け（Ctrl+V）で画像を追加。サンプル画像で試すことも可能 |
| サイズ設定 | 出力サイズ（プリセット／カスタム）、フィットモード、出力形状・角丸、出力形式（JPG/PNG/WebP）・画質、背景色、ファイル名ルールを設定。カスタムサイズは縦横比ロック対応 |
| 配置の確認 | 全画像の自動配置結果をグリッド／リスト表示。個別編集・リセット・削除・追加が可能 |
| 個別編集 | 1枚ずつ位置（ドラッグ）・拡大率（ホイール／スライダー）・回転・反転・ファイル名を調整。9点アライメントグリッドで画像の端をフレームに揃える操作も可能 |
| ダウンロード | 設定内容の要約を確認してダウンロード。1枚の場合は直接ダウンロード、複数枚はZIP生成。書き出し進捗と完了後のサムネイル一覧を表示 |

## SNSプリセット

設定画面にSNS用の出力サイズプリセットを用意しています。

| プリセット名 | サイズ |
|---|---|
| Instagram正方形 | 1080×1080 |
| Instagram縦長（4:5） | 1080×1350 |
| Instagram横長 | 1080×566 |
| ストーリーズ/Reels | 1080×1920 |
| X（投稿） | 1200×675 |
| X（ヘッダー） | 1500×500 |
| Facebookカバー | 851×315 |
...
- [Next.js](https://nextjs.org/) 14（App Router）/ React / TypeScript（strict mode）
- Tailwind CSS（デザイントークンベース、レスポンシブ対応）
...
2. アップロード画面で「サンプル画像で試す」を選択するか、手元のJPG/PNG/WebP画像をドラッグ&ドロップする。
3. サイズ設定画面で出力サイズ・フィットモード・形状・出力形式を設定し、「確認へ進む」を選択する。
4. 配置の確認画面で自動配置結果を確認し、必要に応じて画像をクリックして個別編集画面で調整する。
5. 「ダウンロードへ進む」からダウンロード画面へ遷移し、ダウンロードボタンを選択する。
6. ファイルがダウンロードされ、完了画面に書き出し結果のサムネイルが表示されることを確認する。
